...
#define CLOSE_LOCK_MOTOR_TIME 500
#define SERVO_PULSE_GPIO      35

static mcpwm_cmpr_handle_t comparator;
static esp_timer_handle_t timers;
...
static void timer_callback(void *arg) {
    ...
    // close lock
    int angle = -90;
    //usually 200ms/60degree rotation under 5V power supply
...
void motor_init() {
    ...
    ESP_ERROR_CHECK(esp_timer_create(&timer_args_1, &timers));
}
...
void open_lock() {
    ...
    if (!esp_timer_is_active(timers)) {
        //ESP_LOGI(TAG, "start timer for %d", index);
        esp_timer_start_once(timers, CLOSE_LOCK_MOTOR_TIME * 1000);
    }
}